...
from utils import (
    get_direct_trip_geometry,
    estimate_radius,
    should_use_transit,
    time_to_seconds,
    estimate_radius_and_limit,
    haversine_distance
)
...
def find_direct_trip(origin_coords, dest_coords, search_radius_origin=800, search_radius_dest=800, auto_estimate_radius=False):
    ...
    max_time = min(current_sec + int(estimated_travel_sec) + 3600, current_sec + 10800)
    ...
    cur.execute(
        ...
        "FROM connections WHERE departure_sec BETWEEN %s AND %s AND arrival_sec IS NOT NULL "
        "ORDER BY departure_sec",
        (current_sec, max_time)
    )
    connections = cur.fetchall()

    # --- CSA directo (sin trasbordos) ---
    earliest = {s: current_sec for s in origin_ids}
    trip_used = {s: None for s in origin_ids}
    prev = {}
    best_target = None

    for from_stop, to_stop, dep, arr, trip in connections:
        if from_stop not in earliest or dep < earliest[from_stop]:
            continue
    ...
    if not best_target:
        conn.close()
        del connections, earliest, trip_used, prev
        gc.collect()
    ...
    while cur_stop in prev:
        p = prev[cur_stop]
        path.append((p[0], cur_stop, p[1], p[2], p[3]))
        cur_stop = p[0]
    path.reverse()
    ...
    cur.execute(
        ...
        "r.route_short_name, r.route_long_name, r.route_desc, r.route_url "
        "FROM trips t JOIN routes r ON t.route_id = r.route_id AND t.operator_id = r.operator_id "
        "WHERE t.trip_id = %s LIMIT 1",
        (trip_id,)
    )
    row = cur.fetchone()
    transport = {
...
# -------------------------------
# Función: Find trip with transfer
# -------------------------------
...
def find_trip_with_transfer(origin_coords, dest_coords, search_radius_origin=800, search_radius_dest=1200, transfer_radius=350, auto_estimate_radius=False):
    ...
    straight_distance = haversine_distance(origin_coords[1], origin_coords[0], dest_coords[1], dest_coords[0])
    estimated_travel_sec = (straight_distance / 30000) * 3600
    max_time = min(current_sec + int(estimated_travel_sec) + 3600, current_sec + 10800)
    ...
    cur.execute(
        "SELECT a.stop_id, b.stop_id, CEIL(ST_Distance(a.geom::geography, b.geom::geography)/1.2)::int AS walk_sec "
        "FROM stops a JOIN stops b ON a.stop_id != b.stop_id "
        "WHERE a.stop_id = ANY(%s) AND b.stop_id = ANY(%s) AND ST_DWithin(a.geom::geography, b.geom::geography, %s)",
    ...
    for from_stop, to_stop, dep, arr, trip in connections:
        ...
        if to_stop not in earliest or arr < earliest[to_stop]:
            ...
            for fp_to, walk_sec in footpaths_from.get(to_stop, []):
                ...
                if walk_arr <= max_time and (fp_to not in earliest or walk_arr < earliest[fp_to]):
                    earliest[fp_to] = walk_arr
                    trip_used[fp_to] = None
                    prev[fp_to] = (to_stop, '__walk__', arr, walk_arr)

    if not best_target:
        conn.close()
        del connections, earliest, trip_used, prev, footpaths_from
        gc.collect()
        return {"status": "Not found", "reason": "No trips with transfer"}

    # --- Reconstruir path y legs ---
    path = []
    cur_stop = best_target
    while cur_stop in prev:
        p = prev[cur_stop]
    ...
    cur.execute(
        ...
        "WHERE t.trip_id = ANY(%s)", (trip_ids,)
    )
    transport_map = {
        row[0]: {
            "trip_id": row[0],
    ...
    first_departure = leg1[1][0][3]
    second_departure = leg2[1][0][3]
    total_time = leg2[1][-1][4] - current_sec
